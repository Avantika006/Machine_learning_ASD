...
for i in range(len(df)):
    ...
    if asd_status and adhd_status:
        prediction.append(0)  # Both ASD and ADHD
    elif asd_status:
...
features = df.drop(columns=['subject_sp_id', 'respondent_sp_id', 'family_sf_id', 'biomother_sp_id', 'biofather_sp_id','sex',
                            ...
                            'birth_def_oth_calc', 'birth_oth_calc','etoh_subst','gen_dx_oth_calc_self_report','gen_test_cgh_cma','gen_test_chrom_karyo','gen_test_ep','gen_test_fish_angel','gen_test_fish_digeorge',
                            ...
                            'gen_test_tsc','gen_test_unknown','gen_test_wes','gen_test_wgs','gen_test_oth_calc',
                            'growth_oth_calc','prev_study_calc','eval_year','prev_study_agre','prev_study_asc','prev_study_charge',
                            'prev_study_earli','prev_study_marbles','prev_study_mssng','prev_study_seed','prev_study_ssc','prev_study_vip','prev_study_oth_calc',
                            'neuro_oth_calc','pers_dis','prev_study_oth_calc','psych_oth_calc','schiz',
                            'tics','asd','behav_adhd','age_at_eval_months','age_at_eval_years','gen_test_aut_dd']).copy()
...
# Categorical columns
cat_col = [col for col in features.columns if col != 'gest_age']
