...
for i in range(len(merged_df)):
    ...
    if asd_status and adhd_status:
        prediction.append(2)  # Both ASD and ADHD
        both = both + 1
    elif asd_status:
...
merged_df = merged_df[merged_df['prediction'] != 3].copy()
...
asd_only = merged_df[merged_df['prediction'] == 1]
adhd_only = merged_df[merged_df['prediction'] == 0]
asd_adhd = merged_df[merged_df['prediction'] == 2]

# Randomly sample an equal number of rows from the adhd only individuals subset
asd_only_sampled = asd_only.sample(n=adhd, random_state=42)
...
merged_df_1 = pd.concat([adhd_only, asd_only_sampled])
merged_df_2 = pd.concat([merged_df_1, asd_adhd_sampled])

# Shuffle the rows of the final dataset
merged_df = merged_df_2.sample(frac=1, random_state=42).reset_index(drop=True)
...
le = LabelEncoder()
merged_df['prediction_encoded'] = le.fit_transform(merged_df['prediction'])

# Separate features (X) and target variable (y)
X = features
y = merged_df['prediction_encoded']

# Drop the previously created 'prediction' column
merged_df.drop(columns=['prediction'], inplace=True)
