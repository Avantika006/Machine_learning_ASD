...
for fold, (train_index, test_index) in enumerate(kf.split(X, y), 1):
    ...
    X_train, X_val, y_train, y_val = X.iloc[train_index[:int(0.8 * len(train_index))]], X.iloc[
        train_index[int(0.8 * len(train_index)):]], y[train_index[:int(0.8 * len(train_index))]], y[
    ...
    # Transforming the validation and testing sets
    X_val_transformed = preprocessor.transform(X_val)
    X_test_transformed = preprocessor.transform(X_test)

    mod.fit(X_train_transformed, y_train)
    y_train_pred = mod.predict(X_train_transformed)  # Predictions on training set
    y_val_pred = mod.predict(X_val_transformed)  # Predictions on **validation set** (use y_val)

    train_error = 1 - accuracy_score(y_train, y_train_pred)  # Calculate train error
    val_error = 1 - accuracy_score(y_val, y_val_pred)  # Calculate validation error (use y_val_pred)

    train_errors.append(train_error)
    val_errors.append(val_error)

    # Calculating the accuracy_score for each fold
    accuracy[fold] = accuracy_score(y_val, y_val_pred)
    ...
    f1[fold] = f1_score(y_val, y_val_pred, average='weighted')

    # Calculating the Precision
    precision[fold] = precision_score(y_val, y_val_pred, average='weighted')
